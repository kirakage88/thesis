...
- [ ] 🔴 **Add operational definition of Normal/Abnormal** — `1.1 / 3.1`
...
- [ ] 🔴 **Rewrite research questions for unsupervised focus**
...
- [ ] 🔴 **Rewrite hypotheses (falsifiable with margins and tests)**
...
- [ ] 🔴 **Reframe positioning as complementary triage** — `1.1`
...
- [ ] 🟠 **Update scope and limitations for non-invasive approach** — `1.5`
...
- [ ] 🟡 **Justify unsupervised over supervised** — `1.5 or 3.3`
...
- [ ] 🔴 **Add unsupervised methodology justification** — `3.3`
...
- [ ] 🟠 **Define labeling and validation protocol** — `3.1.6`
...
- [ ] 🟡 **Standardize portable kit mounting protocol** — `3.1.5`
...
- [ ] 🟡 **Add time-since-last-cleaning covariate** — `3.1.5`
...
- [ ] 🔴 **🔧 Verify MPU6050 calibration offsets**
  - From calibration CSV: AX_BIAS = -0.005124, AY_BIAS = -0.265409, AZ_BIAS = 0.379842.
...
- [ ] 🔴 **💻 Calibration factor integration**
  ...
  - ACS712: `ADC_raw − ADC_Zero_Offset` → calibrated current via sensitivity factor.

- [ ] 🟠 **💻 Data formatting script**
  - Master ESP32 outputs structured CSV line: `timestamp,unit_id,set_temp,temp,humid,pressure,inlet,outlet,coil,comp_current,fan_current,voltage,accel_x,accel_y,accel_z`.
  - Ingest script: reads Serial, validates format and ranges, writes to `02_Data/raw/` with naming convention: `YYYYMMDDTHHmmSSZ_<Location>_<UnitID>_<TrialNum>_RAW.csv`.
...
- [ ] 🔴 **📊 Zero-offset calibration** — both ACS712 channels
  ...
  - Compute: `ADC_Zero_Offset = mean(1,000 readings)`.
...
- [ ] 🔴 **📊 3rd-order polynomial calibration**
  ...
  - Fit 3rd-order polynomial per module: `V_actual = a₀ + a₁·ADC + a₂·ADC² + a₃·ADC³`.
...
- [ ] 🟠 **💻 Calibration database script**
  ...
  - Version control in `01_Sensor_Calibration/`.
...
- [ ] 🔴 **📝 Define unsupervised model architectures**
  - **Autoencoder:** Input (9 sensor features: set_temp + 8 calibrated sensor readings) → Encoder (Dense 64 ReLU → Dense 32 ReLU → Dense 16 latent) → Decoder (Dense 32 ReLU → Dense 64 ReLU → Dense 9 linear). Reconstruction error (MSE) as anomaly score. Threshold: 95th percentile of training reconstruction error.
  - **Isolation Forest:** n_estimators [100, 200, 500], contamination [auto, 0.01, 0.05, 0.10], max_samples [256, auto]. Anomaly score: path length.
  ...
  - **GMM:** n_components [1, 2, 3, 5, 10] (selected via BIC). Anomaly score: negative log-likelihood.
...
- [ ] 🟠 **📝 Specify hyperparameter search space** per model
  - Isolation Forest: n_estimators [100, 200, 500], contamination [0.01, 0.05, 0.10].
  - One-Class SVM: nu [0.01, 0.05, 0.10], gamma [scale, auto, 0.01, 0.1].
  - GMM: n_components [1, 2, 3, 5, 10], covariance_type [full, tied, diag, spherical].
  - Autoencoder: latent_dim [8, 16, 32], learning_rate [0.001, 0.0001], epochs [50, 100], batch_size [32, 64].

- [ ] 🟡 **📝 Add overfitting mitigation section**
  - Autoencoder: L2 regularization, dropout in encoder, early stopping (patience=10).
  - Isolation Forest: max_samples, limited tree depth.
  - One-Class SVM: nu parameter controls outlier fraction.
  - GMM: BIC for model selection.

- [ ] 🟡 **📝 Define evaluation metrics for unsupervised models**
  - Anomaly score separation (known-fault vs. known-healthy fleet): Mann-Whitney U test.
  - True positive rate on injected faults: ≥ 80%.
  ...
  - Confound regression: anomaly score vs. ambient temp, humidity, time-since-cleaning, kit ID → must show no significant correlation (p > 0.05).
...
- [ ] 🟡 **💻 Project scaffolding**
  - Create `03_Code/` directory structure (ingest, preprocessing, modeling, evaluation subdirs).
...
- [ ] 🔴 **🤖 Train convolutional autoencoder**
  ...
  - Save best model (lowest val_loss) as `cnn_vibration_autoencoder.pth`.
  - Output per spectrogram: reconstruction error (anomaly score) + latent feature vector.

- [ ] 🟠 **🤖 Vibration anomaly detection evaluation**
...
- [ ] 🔴 **🤖 Complete preprocessing pipeline**
  ...
  - Features: set_temp + 8 calibrated sensor readings + vibration anomaly score from CNN + vibration statistical features (RMS, kurtosis, peak frequency) + time-since-last-cleaning.
  ...
  - Output: X_all, metadata (unit_id, timestamp, kit_id, location) saved to `02_Data/processed/`.
...
- [ ] 🔴 **🤖 Train Isolation Forest**
  - Hyperparameter tuning via grid search (contamination, n_estimators, max_samples).
  - Save `isolation_forest.pkl`.
...
- [ ] 🔴 **🤖 Train One-Class SVM**
  ...
  - Save `one_class_svm.pkl`.

- [ ] 🔴 **🤖 Train Gaussian Mixture Model**
  - Select n_components via BIC (test 1-10 components).
  - Save `gmm.pkl`.
...
- [ ] 🔴 **🤖 Layer 1 — Retrospective log validation**
  - Deploy kit to known-fault units (1-2 visits each).
  - Anomaly score distributions: known-fault vs. known-healthy fleet.
  - Mann-Whitney U test: p < 0.05 for score separation.
  - Report: mean anomaly score per group, effect size (Cohen's d).

- [ ] 🔴 **🤖 Layer 2 — Controlled fault injection validation**
  - Run all 4 models on pre-fault → post-fault → recovery data.
  - True positive rate: anomaly score must rise post-injection for each fault type.
  - Target: ≥ 80% detection rate across injected faults.
  - False positive rate on known-healthy fleet ≤ 15%.

- [ ] 🔴 **🤖 Layer 4 — Confound verification**
  - Regress anomaly score against: ambient temperature, ambient humidity, time-since-last-cleaning, kit ID.
  - Must show no significant correlation (p > 0.05 for each confound).
  - If correlation exists → include confound as feature, re-run, report both.
...
- [ ] 🟠 **🤖 Model comparison**
  ...
  - Ranking: which model performs best overall?

- [ ] 🟡 **🤖 Unit-level generalization**
  - Leave-One-Unit-Out evaluation: train on all but one unit, score the held-out unit.
...
- [ ] 🟠 **📝 Write Chapter 4 anomaly detection results**
  - Per layer: validation results table, statistical test outputs.
  - Model comparison table with all metrics.
  - Confound verification: regression results, significance.
...
- [ ] 🔴 **🚀 Watchdog automation setup**
  - Monitor `02_Data/raw/` for new files (use staging directory + atomic move to prevent race conditions).
...
- [ ] 🔴 **💻 Automated anomaly scoring script**
  - Load best model (e.g., `isolation_forest.pkl`).
  - Load scaler + PCA transformer (fit during Phase 8).
  - Process: set_temp + 8 sensor readings → calibrated → standardized → PCA → model.anomaly_score().
  - Store: `{unit_id, timestamp, anomaly_score, sensor_snapshot}` → `anomalies.db`.
  - Error handling: catch NaN, malformed input, shape mismatch → log error → skip file.

- [ ] 🟠 **🚀 SQLite anomalies database**
  - Schema: `anomalies` table (id, unit_id, timestamp, anomaly_score, model_name, sensor_json).
  - Schema: `alerts` table (id, unit_id, timestamp, severity, anomaly_score_threshold_exceeded, acknowledged).
...
- [ ] 🔴 **📝 Discussion per section**
  ...
  - Were the hypotheses supported? (H1-H4, each with statistical test results).
...
| DIY airflow sensor not ready in time (PMV experiment) | High | Low | **Already mitigated** — PMV is conditional. If sensor fails, H4 and RQ3 drop silently. Does not affect primary thesis contribution. |
